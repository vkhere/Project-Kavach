...
```mermaid
graph TD
    A[Outside Attacker] -->|Step 1| B[Exploits Stored XSS F-02 on Customer Portal]
    B -->|Step 2| C[Steals Administrator Session Cookie]
    C -->|Step 3| D[Gains Foothold on Internal Administrator Workstation 10.5.26.132]
    D -->|Step 4| E["Executes Trickbot Payload (Recon IP Lookup via api.ipify.org - Network Frame 818)"]
    E -->|Step 5| F["Lateral Movement via SMB svcctl to Domain Controller 10.5.26.4 (Network Frame 10799)"]
    F -->|Step 6| G["Active Directory Domain Compromised & C2 Heartbeat Started to 37.228.70.134"]
