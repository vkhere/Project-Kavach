...
## 1. PCAP Selection & Analogue Justification
**Source Capture:** `2021-05-26-Trickbot-infection-with-Cobalt-Strike.pcap` (from Malware-Traffic-Analysis.net).

**Justification:** This capture was explicitly selected as a defensible analogue for the Meridian FinServe breach scenario. It provides a high-fidelity representation of a multi-stage enterprise compromise, exhibiting all requisite threat behaviors mandated by the engagement brief: automated data exfiltration (Trickbot), persistent C2 beaconing (Cobalt Strike), and internal lateral movement toward a Domain Controller via SMB/RPC. These behaviors perfectly mirror the "anomalous east-west and outbound traffic" reported by Meridian's SOC, making it an ideal dataset for reconstructing the incident. Furthermore, this public PCAP can be fully analyzed using Wireshark and `tshark` locally, satisfying the strict engagement constraint that all tooling must be "free-tier or open-source only" and run entirely on a personal laptop.

---

## 2. Executive Summary of Network Findings
A forensic review of the 72-hour incident packet capture was conducted to trace anomalous traffic patterns. The analysis confirmed a staged infection flow matching a banking trojan (Trickbot) lateral movement and Cobalt Strike beaconing campaign:
...
## 3. Evidence Mapping (MITRE ATT&CK)
...
## 4. Architecture Proposal (Before vs. Proposed)
